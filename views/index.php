<?php require_once __DIR__.'/_header.php'; ?>

<main class="w-full h-screen-80 px-4 md:px-12 lg:px-44 bg-black pt-46">
  <div class="">
<h1 class="text-gray-50 text-8xl font-exo sm:text-8xl">FoodFly</h1>
  <p class="text-gray-50">Get your food in a minute</p>
  </div>

    <div class="">
      <div class="flex pt-4">
        <a href="login"><button class="bg-teal-200 text-gray-900 rounded-3xl py-2 px-8 my-4 sm:text-sm">Customer</button></a>
        <a href="login"><button class="bg-zinc-800 text-gray-50 rounded-3xl py-2 px-8 my-4 mx-4 sm:mx-2 sm:text-sm">Partner</button></a>
      </div>
      <div class="flex py-2">
            <p class="text-sm text-gray-50 opacity-60"><a href="signup">Sign up?</a></p>
            <p class="text-sm text-gray-50 px-4 opacity-60"><a href="login">Admin?</a></p>
      </div>
    </div>

    
</main>

<?php require_once __DIR__.'/_footer.php'  ?>






